...
class VirtualKeyboard {
  ...
  keyDownHandler(key) {
    ...
    this.current.code === 'MetaLeft'
      ? (
        ...
        const KEYACTIVE = document.querySelector(`div>.${key.code}`);
        KEYACTIVE.classList.add('animated');
  }
  ...
  keyUpHandler(key) {
    ...
    this.toggleCase()));
    const KEYACTIVE = document.querySelector(`div>.${key.code}`);
    KEYACTIVE.classList.remove('animated');
  ...
  mouseDownHandler(key) {
    ...
    const KEYMOUSE = key.path[2];
    KEYMOUSE.classList.add('animated');
  }
  ...
  mouseUpHandler(key) {
    ...
    const KEYMOUSE = key.path[2];
    KEYMOUSE.classList.remove('animated');
